...
```
(6pi,j,k−pi+1,j,k−pi−1,j,k−pi,j+1,k−pi,j−1,k−pi,j,k+1−pi,j,k−1) = −Δx2Δtρ(∇⋅u)
```
...
```
[2,−1,−10,−1][p1,p2,p3,p4] = −Δx2Δtρ[∇⋅u1∇⋅u2∇⋅u3∇⋅u4]
```

In this example, suppose `Δx=1, Δt=0.1, ρ=1`. In this case, b is
...
`p = [−3.375,−1.125,3.375,1.125]`

Now that we have values for p, we can compute our new velocity field.

```
u1,0,0 = u∗1,0,0−ΔtΔxρ(−1.125+3.375) = −0.225
u1,1,0 = u∗1,1,0−ΔtΔxρ(1.125−3.375) = 0.225
v0,1,0 = u∗0,1,0−ΔtΔxρ(3.375+3.375) = 0.225
