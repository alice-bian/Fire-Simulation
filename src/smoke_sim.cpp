...
void SmokeSim::step()
{
   ...
   // Step0: Gather user forces
	...
	// Dump out rendering particle data in .obj file
	std::string particleFile = "Output\\frame" + std::to_string(mTotalFrameNum) + ".obj";
	mGrid.saveParticle(particleFile);

	string colorFile = "OutputColors\\colorFrame " + std::to_string(mTotalFrameNum) + ".obj";
	mGrid.saveColors(colorFile);

	std::cout << "FRAME NUM: " << mTotalFrameNum << std::endl;
